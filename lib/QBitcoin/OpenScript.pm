...
sub script_for_address {
    my $class = shift;
    my ($address) = @_;
    return OP_DUP . OP_HASH160 . op_pushdata(pubhash_by_address($address)) . OP_EQUALVERIFY . OP_CHECKSIG;
}

sub check_input {
    my ($class) = shift;
    my ($open_script, $close_script, $tx, $input_num) = @_;
    my $res = script_eval($close_script, $open_script, $tx, $input_num);
