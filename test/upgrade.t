...
my $upgrade_script = QBT_BURN_SCRIPT . pack("C", length($open_script)) . $open_script . OP_DROP;
...
my $up = QBitcoin::Coinbase->new({
    ...
    value            => $value,
    upgrade_level    => 0,
    scripthash       => substr($btc_tx->out->[$btc_out_num]->{open_script}, 1+QBT_BURN_SCRIPT_LEN, length($btc_tx->out->[$btc_out_num]->{open_script}) - 2 - QBT_BURN_SCRIPT_LEN),
});
